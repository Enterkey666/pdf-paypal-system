...
# xhtml2pdfのオプションインポート（App Engine対応）
try:
    from xhtml2pdf import pisa
    XHTML2PDF_AVAILABLE = True
    print("✓ xhtml2pdf インポート成功")
except ImportError as e:
    XHTML2PDF_AVAILABLE = False
    print(f"⚠ xhtml2pdf インポート失敗: {e}")
    # フォールバック関数
    def pisa():
        return None
        ...
        try:
            ...
            if e.errno == 30:  # Read-only file system
                ...
                if folder == app.config['RESULTS_FOLDER']:
                    app.config['RESULTS_FOLDER'] = temp_folder
                elif folder == app.config['UPLOAD_FOLDER']:
                    app.config['UPLOAD_FOLDER'] = temp_folder
                app.logger.warning(f"読み取り専用ファイルシステムのため一時ディレクトリを使用: {temp_folder}")
            else:
